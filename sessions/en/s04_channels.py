...
class TelegramChannel(Channel):
    ...
    def _flush_all(self) -> list[InboundMessage]:
        ...
        # Flush text buffer after 0.2s silence (reduced from 1.0s for lower latency)
        expired_txt = [k for k, b in self._text_buf.items() if (now - b["ts"]) >= 0.2]
        ...
        if finish_reason == "stop":
            ...
            for tc in message.tool_calls:
                ...
                messages.append({
                    "role": "tool",
                    "tool_call_id": tc.id,
                    "content": process_tool_call(tc.function.name, tool_input),
                })
        else:
            text = message.content or ""
            if text:
