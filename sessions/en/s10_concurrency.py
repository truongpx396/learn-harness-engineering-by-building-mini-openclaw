...
Requires: OPENAI_API_KEY, MODEL_ID (configured in .env)
            ...
            def _turn() -> str:
                ...
                while True:
                    ...
                    if finish_reason == "stop":
                        ...
                        for tc in message.tool_calls:
                            print_info(f"  [tool: {tc.function.name}]")
                            msgs.append({
                                "role": "tool",
                                "tool_call_id": tc.id,
                                "content": tool_handler(tc.function.name, json.loads(tc.function.arguments)),
                            })
                    else:
                        final_text = message.content or ""
                        break
