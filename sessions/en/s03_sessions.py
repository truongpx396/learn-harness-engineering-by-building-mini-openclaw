...
class SessionStore:
    ...
    def _rebuild_history(self, path: Path) -> list[dict]:
        """
        Rebuild OpenAI-format messages from JSONL lines.
        - user records       → role=user
        - assistant records  → role=assistant (content string)
        - tool_use records   → append to assistant message's tool_calls list
        - tool_result records → role=tool with tool_call_id
        """
        messages: list[dict] = []
        ...
        for line in lines:
            ...
            if rtype == "user":
                ...
                content = record["content"]
                # Flatten legacy Anthropic-style content block lists to string
...
def _serialize_messages_for_summary(messages: list[dict]) -> str:
    """Flatten OpenAI-format messages to plain text for LLM summarization."""
    parts: list[str] = []
    for msg in messages:
        role = msg["role"]
        ...
        for tc in msg.get("tool_calls") or []:
            fn = tc.get("function", {})
            args_preview = fn.get("arguments", "")[:500]
...
class ContextGuard:
    ...
    def _truncate_large_tool_results(self, messages: list[dict]) -> list[dict]:
        """Walk through messages and truncate oversized tool result content."""
        result = []
        for msg in messages:
...
def agent_loop() -> None:
    ...
    while True:
        ...
        while True:
            ...
            store.save_turn("assistant", message.content or "")
