...
## Test Gateway
The gateway uses JSON-RPC 2.0. You can test it with `wscat` or Python:

**Option 1 — wscat (easiest)**
```bash
...
{"jsonrpc":"2.0","id":3,"method":"send","params":{"text":"Hello!","channel":"cli","peer_id":"test-user"}}
{"jsonrpc":"2.0","id":4,"method":"bindings.list","params":{}}
```
...
async def test():
    async with websockets.connect('ws://localhost:8765') as ws:
        req = json.dumps({'jsonrpc':'2.0','id':1,'method':'send','params':{'text':'Hello Luna!','channel':'cli','peer_id':'tester'}})
        await ws.send(req)
        print(await ws.recv())
...
**Option 3 — Send to a specific agent (sage)**
```json
{"jsonrpc":"2.0","id":5,"method":"send","params":{"text":"What is 2+2?","agent_id":"sage","channel":"cli","peer_id":"tester"}}
```

The available methods are: `send`, `bindings.set`, `bindings.list`, `sessions.list`, `agents.list`, `status`.
